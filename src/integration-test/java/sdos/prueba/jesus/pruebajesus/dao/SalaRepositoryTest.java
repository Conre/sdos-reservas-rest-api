...
public class SalaRepositoryTest {
    ...
    @Test
    void givenIdSalaAndUpdatedSala_whenUpdateSala_thenReturnNothing() {
        Sala salaUpdated = new Sala("1234", "Sala 2", 3, Sets.newHashSet(Arrays.asList(new RecursoTecnico("12", "Televisor"), new RecursoTecnico("13", "Proyector"), new RecursoTecnico("14", "Pizarra digiral"))));

        Sala sala = new Sala("1234", "Sala 1", 3, Sets.newHashSet(Arrays.asList(new RecursoTecnico("12", "Televisor"), new RecursoTecnico("13", "Proyector"))));
        entityManager.persist(sala);
        Optional<Sala> find = this.salaRepository.findById("1234");

        find.get().setNombre(salaUpdated.getNombre());
        find.get().setRecursosTecnicos(salaUpdated.getRecursosTecnicos());

        entityManager.persist(find.get());

        Optional<Sala> updated = this.salaRepository.findById("1234");

        assertEquals(updated.get(), salaUpdated);
    }
}
